...
- **宿主 token 总量降约 31%,其中最贵的 *output* token 降约 65%** —— output
  按 $25/M 计价,是最贵的一项,委派把它几乎全部推给 kiro。宿主省下的 token
  开销转化为 kiro credit($0.04/个),而 credit 仅占委派总成本的约 11% ——
  成本大头仍是宿主端 token,主要来自插件装载与验证。
- **三任务合计便宜约 9%**($1.367 vs $1.502),但优势**与任务规模相关**:小型
  单函数任务委派反而贵约 15%(固定开销摊不薄),中大型任务则便宜约 17%。结论:
