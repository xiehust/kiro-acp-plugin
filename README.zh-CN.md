...
把实现委派给 kiro,等于把编码里最贵的部分 —— 反复读/改/测/修的多轮循环 ——
从宿主模型移到按 credit 计费的 kiro agent 上,宿主只保留便宜的工作;**到底有多
便宜,取决于委派之后宿主是否还亲自验证 kiro 的产出**。两项受控 A/B 实验(两侧
同为 Opus 4.8,三个由小到大的编码任务,由框架独立运行 `pytest` 作为质量闸门)
划出了这条区间的两端:

- **委派后验证** —— 宿主亲自读文件、重跑 `pytest`、失败再回委派(随附 skill 教的
  正是这种模式):**宿主 token 降约 30%、最贵的 *output* token 降约 65%、合计便宜
  约 8%**($1.48 vs $1.60)。
  [报告](docs/experiments/2026-06-12-token-credit-cost-experiment.md)
- **委派后信任** —— 宿主只发一次委派、信任 kiro 自己的测试结果,不读文件也不跑
  `pytest`:**宿主 token 降约 71%、output token 降约 87%、合计便宜约 48%**($1.05
  vs $2.03)。验证往返**本就是宿主成本的大头**,去掉它就把成本砍掉约一半 —— 但这
  也撤掉了宿主侧的安全网:正确性此时**完全押在 kiro 的自测上**。
  [报告](docs/experiments/2026-06-16-token-credit-cost-noverify-experiment.md)

两次实验里,委派推给 kiro 的都是最贵的 output($25/M);宿主剩下的开销转化为 kiro
credit($0.04/个),仅占委派总成本的 **~9–19%**。**两次质量都不打折** —— 每个臂
都通过独立 `pytest`,委派臂也都达到了要求的测试覆盖。无验证那次质量能保住,是
**因为有这道独立闸门替它确认**;真实使用中通常没有这种外部闸门,所以除非你下游有
CI 或别的检查兜底,**优先选"委派后验证"**。

![kiro](docs/assets/krio-experiment.png)

所以这个插件的价值不是"永远更便宜",而是:对于非平凡、可清晰描述验收标准的实现型
任务,它把开销从你的 Opus token 池转移到 kiro 的 credit 池,并削减最贵的 output
token。验证是那个旋钮:留着它换一道宿主侧安全网(便宜约 8%),或在有外部闸门兜底
时去掉它把成本再砍一半(便宜约 48%)。数据为每格 n=1 的分钟级任务;局限性详见
报告。

> **v0.4.4 —— 上下文开销优化。** 后续确定性核算了插件真正常驻 context 的开销:
> ~534 token(3 个 MCP 工具定义 + skill/agent 描述),通过把操作细节移出 registry
> 描述,精简到 **~437 token(−18%)**。同时纠正了报告早先的一处误判:两臂的
> cache write 差异**并非"插件装载"**(常驻面很小,且插件全局安装、两臂都加载)——
> 而是委派的多轮往返、更长提示词、以及 cache TTL 重写时机。该优化真实但低于试验的
> run-to-run 噪声,故不改变上面的成本数字(报告 §10)。
