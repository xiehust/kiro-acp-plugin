...
从宿主模型移到按 credit 计费的 kiro agent 上,宿主只保留便宜的工作(发起任务 +
验证结果)。一项受控 A/B 实验(两侧同为 Opus 4.8,三个由小到大的编码任务,由
框架独立运行 `pytest` 验证 ——
[完整报告](docs/experiments/2026-06-12-token-credit-cost-experiment.md))表明:

- **宿主 token 总量降约 30%,其中最贵的 *output* token 降约 65%** —— output
  按 $25/M 计价,是最贵的一项,委派把它几乎全部推给 kiro。宿主省下的 token
  开销转化为 kiro credit($0.04/个),而 credit 仅占委派总成本的约 9% ——
  成本大头仍是宿主端 token,主要来自验证轮次(并非插件:其常驻上下文仅 ~440 token,见下)。
- **三任务合计便宜约 8%**($1.48 vs $1.60),但优势**与任务规模相关**:小型
  单函数任务基本打平(方向随机翻转 —— 固定开销摊不薄后,差异落在 LLM 轨迹噪声
  内),中大型任务则是稳定的赢家。结论:把有分量的任务委派出去,琐碎的两种方式
  成本相当、直接自己做更省事。
...
token,并让宿主上下文保持精简(宿主只看到一次发起 + 一次验证,而不是完整的实现
过程)。数据为每格 n=1 的分钟级任务;局限性详见报告。
